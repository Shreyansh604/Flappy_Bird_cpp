	...
	should_fly(false)
{
	...
	bird_sprite.setScale(sf::Vector2f(SCALE_FACTOR, SCALE_FACTOR));
...
void Bird::update(sf::Time& dt)
{
	if (bird_sprite.getGlobalBounds().top < 548 && should_fly)
	{
		...
		bird_sprite.move(sf::Vector2f(0, velocity_y));

		if (bird_sprite.getGlobalBounds().top < 0)
		{
			bird_sprite.setPosition(sf::Vector2f(100, 0));
		}
	}
}
...
{
	return bird_sprite.getGlobalBounds().left;
}
